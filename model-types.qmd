...
(2) Independence between observations (i.e., the errors are uncorrelated)
...
*We can check for overdispersion using randomized quantile residuals*
...
**Important Note:** since overdispersed models no longer specify a distribution of the outcome conditional on the covariates, tests based on the log-likelihood are no longer valid and we can no longer use randomized quantile residuals or do line-ups. *We can still still do Wald Tests and interpret coefficients using the new Confidence Interval*
...
*Impute if clear value that should be filled in. Also, can impute to mean if needed (use just mean for time sake or if there is not much variability in the covariate, though can lead to biased estimates)*
